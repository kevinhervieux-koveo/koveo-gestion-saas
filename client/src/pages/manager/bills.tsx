...
function BillsPage({ buildingId, organizationId }: BillsProps) {
  ...
  // Handle auto-generated bill click
  const handleAutoGeneratedBillClick = async (bill: Bill) => {
    try {
      // Fetch template data from the API
      const response = await fetch(`/api/bills/${bill.id}/template-data`, {
        credentials: 'include',
      });
      ...
      const { templateData, sourceInfo } = await response.json();
      
      // Set the selected bill with pre-populated data
      setSelectedAutoGeneratedBill({
        ...bill,
        templateData,
        sourceInfo,
      });
      ...
      toast({
        ...
        description: `Form pre-populated with data from "${sourceInfo.title}"`,
      });
    } catch (error) {
