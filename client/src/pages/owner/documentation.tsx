  ...
  const { data: docData, isLoading, refetch, isFetching } = useQuery<DocumentationData>({
    ...
    queryFn: () => {
      ...
      return Promise.resolve({
        ...
        dependencies: [
          { name: "React", version: "18.x", type: "production", description: "Frontend UI library" },
          { name: "TypeScript", version: "5.x", type: "development", description: "Type-safe JavaScript" },
          { name: "Express", version: "4.x", type: "production", description: "Web application framework" },
          { name: "PostgreSQL", version: "16.x", type: "production", description: "Relational database" },
          { name: "Drizzle ORM", version: "0.x", type: "production", description: "TypeScript ORM" }
        ]
      });
    }
  });
...
`;
  ...
  const generateDependenciesList = (data: DocumentationData | undefined) => {
    ...
    data.dependencies.filter(d => d.type === 'production').forEach(dep => {
      doc += `- ${dep.name} (${dep.version}): ${dep.description}\n`;
    });
    doc += '\nDEVELOPMENT DEPENDENCIES:\n';
    data.dependencies.filter(d => d.type === 'development').forEach(dep => {
      doc += `- ${dep.name} (${dep.version}): ${dep.description}\n`;
    });
    return doc;
  };
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-gray-600">
                  Export comprehensive documentation in human-readable formats including Word documents, Excel spreadsheets, and other Google Suite compatible files.
                </p>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-gray-600">
                  Generate exhaustive technical documentation optimized for AI/LLM processing. This provides a complete portrait of the application architecture, components, and business logic.
                </p>
