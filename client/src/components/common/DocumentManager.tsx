...
import { Upload, Download, Edit, Trash2, FileText, Search, Plus, Building, Calendar, ArrowLeft, ChevronLeft, ChevronRight } from "lucide-react";
...
const createDocumentFormSchema = (type: 'building' | 'residence') => {
  ...
  if (type === 'building') {
    ...
    return z.object({
      ...baseSchema,
      type: z.enum(['lease', 'inspection', 'maintenance', 'legal', 'insurance', 'financial', 'communication', 'photos', 'other']),
...
function EditDocumentForm({ document, config, onSave, onCancel }: EditDocumentFormProps) {
  ...
  const editForm = useForm({
    ...
    defaultValues: {
      ...
      ...(config.type === 'building' ? { buildingId: document.buildingId } : { residenceId: document.residenceId }),
...
export default function DocumentManager({ config }: DocumentManagerProps) {
  ...
  const { data: entity } = useQuery({
    ...
    queryFn: () => config.entityId ? apiRequest("GET", `/api/${config.type}s/${config.entityId}`) as Promise<any> : Promise.resolve(null),
  ...
  const queryParam = config.type === 'building' 
    ? `buildingId=${config.entityId}`
    : `residenceId=${config.entityId}`;

  const { data: documents = [], isLoading: documentsLoading } = useQuery({
    queryKey,
    queryFn: () => config.entityId ? apiRequest("GET", `/api/documents?${queryParam}`) as Promise<Document[]> : Promise.resolve([]),
    enabled: !!config.entityId,
  });
  ...
  if (!config.entityId) {
    return (
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header title={`${config.type === 'building' ? 'Building' : 'Residence'} Documents`} subtitle="Entity ID is required" />
        <div className="flex-1 overflow-auto p-6">
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              <Building className="h-12 w-12 text-gray-400 mb-4" />
              <h3 className="text-lg font-medium text-gray-900 mb-2">{config.type === 'building' ? 'Building' : 'Residence'} ID Required</h3>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <Form {...form}>
                      <form onSubmit={form.handleSubmit(handleCreateDocument)} className="space-y-4">
                        ...
                        <DialogFooter>
                          ...
                          >
                            {createDocumentMutation.isPending || isUploadingNewFile ? "Creating..." : "Create Document"}
                          </Button>
                        </DialogFooter>
                      </form>
                    </Form>
                          ...
                          <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              {categoryDocuments.map((document) => (
                                <Card key={document.id} className="cursor-pointer hover:shadow-md transition-shadow" data-testid={`document-card-${document.id}`}>
                                  <CardContent className="p-4">
                                    <div className="flex items-start justify-between mb-2">
                                      <h4 className="font-medium text-sm truncate flex-1 mr-2" data-testid={`document-name-${document.id}`}>
                                        {document.name}
                                      </h4>
                                    ...
                                    <p className="text-xs text-gray-500 mb-2" data-testid={`document-date-${document.id}`}>
      ...
      <Dialog open={isViewDialogOpen && isEditMode} onOpenChange={(open) => {
