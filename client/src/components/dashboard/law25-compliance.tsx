...
import { Shield, AlertTriangle, CheckCircle, XCircle, FileText, Users, Lock, Globe, Eye, Scale } from 'lucide-react';
...
export function Law25Compliance() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'dataCollection': return FileText;
      case 'consent': return Users;
      case 'dataRetention': return FileText;
      case 'security': return Lock;
      case 'crossBorderTransfer': return Globe;
      case 'dataSubjectRights': return Eye;
  ...
  return (
    <div className="space-y-6">
      {/* Compliance Overview */}
      <Card className={`border-l-4 ${complianceData.complianceScore >= 80 ? 'border-l-green-500' : complianceData.complianceScore >= 60 ? 'border-l-yellow-500' : 'border-l-red-500'}`}>
        <CardHeader>
            ...
            <Card key={category} className={`${isClean ? 'border-green-200' : 'border-red-200'}`}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge variant={isClean ? 'default' : 'destructive'}>
                    {count}
                  </Badge>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {complianceData.violations.slice(0, 10).map((violation, index) => (
