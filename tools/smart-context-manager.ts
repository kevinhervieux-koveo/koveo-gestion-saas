...
export class SmartContextManager {
  ...
  private calculateImportance(filePath: string, content: string): number {
    ...
    if (filePath.includes('app.') || filePath.includes('main.')) {
      importance += 30;
    }
  ...
  public updateWorkingSet(files: string[], focusArea?: string): void {
    this.workspaceContext.workingSet = [
      ...new Set([...this.workspaceContext.workingSet, ...files]),
    ];
