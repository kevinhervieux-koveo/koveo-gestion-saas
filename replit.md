...
### August 2025
- **2025-08-18**: Successfully resolved roadmap features API issue that was preventing features from displaying in the admin roadmap page. Root cause was Vite middleware intercepting `/api/features` requests before reaching the route handler. Fixed by registering the features API route in server/index.ts before Vite middleware initialization, ensuring API routes take precedence over static file serving. Roadmap now properly displays 112 features across all categories with correct progress calculations.
