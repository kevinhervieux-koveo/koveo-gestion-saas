...
export class BillAutoGenerationService {
  ...
  /**
   * Generate bills for the next year based on a recurrent source bill.
  ...
  generateForNextYear(sourceBill: Bill): Bill[] {
    if (sourceBill.paymentType !== 'recurrent') {
      throw new Error('Only recurrent bills can generate future bills');
    }
    ...
    return dueDates.map((dueDate, index) => ({
      ...
      billNumber: `${sourceBill.billNumber}-${nextYear}-${String(index + 1).padStart(2, '0')}`,
      ...
      isAiAnalyzed: false,
      aiAnalysisData: null,
      notes: `Auto-generated from: ${sourceBill.title}`,
      isAutoGenerated: true,
      sourceTemplateId: sourceBill.id,
      autoGeneratedLabel: this.createAutoGeneratedLabel(sourceBill, nextYear),
