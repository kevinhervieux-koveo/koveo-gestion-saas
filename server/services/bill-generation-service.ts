...
export class BillAutoGenerationService {
  ...
  async createAutoGeneratedBill(templateBill: Bill, newBillData: Partial<Bill>, suggestedDate?: Date): Promise<Bill> {
    if (templateBill.paymentType !== 'auto-generated') {
      throw new Error('Can only create bills from auto-generated templates');
    }
  ...
  async generateNextYearBill(sourceBill: Bill): Promise<Bill | null> {
    try {
      ...
      const autoGeneratedBill = {
        ...
        isAiAnalyzed: false,
        aiAnalysisData: null,
        notes: `Auto-generated from recurrent bill: ${sourceBill.billNumber}`,
        isAutoGenerated: true,
        sourceTemplateId: sourceBill.id,
  ...
  async regenerateAutoGeneratedBills(sourceBillId: string): Promise<Bill[]> {
    try {
      ...
      const newBill = await this.generateNextYearBill(sourceTemplate);
  ...
  /**
   * Generate bill number using pattern: AUTO-{year}-{month}-{category}-{sequence}
   * @param sourceBill - Source bill for context
