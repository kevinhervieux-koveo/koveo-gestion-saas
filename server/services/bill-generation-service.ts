...
export class BillGenerationService {
  ...
  async generateFutureBillInstances(parentBill: Bill): Promise<{
    ...
    if (parentBill.paymentType !== 'recurrent' || !parentBill.schedulePayment) {
    ...
    // Calculate occurrences based on schedule
    const occurrences = this.calculateOccurrences(currentDate, endDate, parentBill.schedulePayment, parentBill.scheduleCustom);
    ...
    for (const occurrenceDate of occurrences) {
      ...
      for (let partIndex = 0; partIndex < paymentParts.length; partIndex++) {
        ...
        const generatedBill = {
          ...
          documentPath: parentBill.documentPath,
          documentName: parentBill.documentName,
          isAiAnalyzed: false,
          aiAnalysisData: null,
          notes: `Auto-generated bill - Part ${partIndex + 1}/${paymentParts.length}`,
          autoGenerated: true,
   ...
   */
  private calculateOccurrences(startDate: Date, endDate: Date, scheduleType: string, customDates?: string[]): Date[] {
    const occurrences: Date[] = [];
    let currentDate = new Date(startDate);

    if (scheduleType === 'custom' && customDates) {
      // Handle custom schedule dates
      return this.calculateCustomOccurrences(startDate, endDate, customDates);
    }
  ...
  private generateBillTitle(parentBill: Bill, occurrenceDate: Date, partIndex: number, totalParts: number): string {
    ...
    if (totalParts > 1) {
      return `${parentBill.title} - ${monthYear} (Part ${partIndex + 1}/${totalParts})`;
    } else {
      return `${parentBill.title} - ${monthYear}`;
    }
  }
