...
export class MemStorage implements IStorage {
  ...
  async getPermissions(): Promise<Permission[]> {
    // In-memory storage doesn't have predefined permissions
    return [];
  }
  ...
  async getRolePermissions(): Promise<RolePermission[]> {
    // In-memory storage doesn't have predefined role permissions
    return [];
  }
  ...
  async getUserPermissions(): Promise<UserPermission[]> {
    // In-memory storage doesn't have predefined user permissions
    return [];
  }
  ...
  // Contact operations - required by IStorage interface
  /**
   * Retrieves all contacts from storage.
   */
   ...
   * Retrieves contacts for a specific residence with user details.
   */
  async getContactsForResidence(_residenceId: string): Promise<Array<Contact & { user: User }>> {
    return [];
  }
  ...
  // Organization operations - required by IStorage interface
  ...
  async getOrganization(_id: string): Promise<Organization | undefined> {
    return this.organizations.get(_id);
  }

  async getOrganizationByName(_name: string): Promise<Organization | undefined> {
    return Array.from(this.organizations.values()).find(org => org.name === _name);
  }

  async createOrganization(_organization: InsertOrganization): Promise<Organization> {
    const id = randomUUID();
    const organization: Organization = {
      ..._organization,
      id,
  ...
  async updateOrganization(_id: string, _updates: Partial<Organization>): Promise<Organization | undefined> {
    const existing = this.organizations.get(_id);
    if (!existing) return undefined;
    const updated = { ...existing, ..._updates, updatedAt: new Date() };
    this.organizations.set(_id, updated);
    return updated;
  }

  // Building operations - required by IStorage interface
  async getBuildings(): Promise<Building[]> {
    return Array.from(this.buildings.values());
  }

  async getBuilding(_id: string): Promise<Building | undefined> {
    return this.buildings.get(_id);
  }

  async getBuildingsByOrganization(_organizationId: string): Promise<Building[]> {
    return Array.from(this.buildings.values()).filter(b => b.organizationId === _organizationId);
  }

  async createBuilding(_building: InsertBuilding): Promise<Building> {
    const id = randomUUID();
    const building: Building = {
      ..._building,
      id,
  ...
  async updateBuilding(_id: string, _updates: Partial<Building>): Promise<Building | undefined> {
    const existing = this.buildings.get(_id);
    if (!existing) return undefined;
    const updated = { ...existing, ..._updates, updatedAt: new Date() };
    this.buildings.set(_id, updated);
    return updated;
  }

  async deleteBuilding(_id: string): Promise<boolean> {
    return this.buildings.delete(_id);
  }

  // Residence operations - required by IStorage interface
  async getResidences(): Promise<Residence[]> {
    return Array.from(this.residences.values());
  }

  async getResidence(_id: string): Promise<Residence | undefined> {
    return this.residences.get(_id);
  }

  async getResidencesByBuilding(_buildingId: string): Promise<Residence[]> {
    return Array.from(this.residences.values()).filter(r => r.buildingId === _buildingId);
  }

  async createResidence(_residence: InsertResidence): Promise<Residence> {
    const id = randomUUID();
    const residence: Residence = {
      ..._residence,
      id,
  ...
  async updateResidence(_id: string, _updates: Partial<Residence>): Promise<Residence | undefined> {
    const existing = this.residences.get(_id);
    if (!existing) return undefined;
    const updated = { ...existing, ..._updates, updatedAt: new Date() };
    this.residences.set(_id, updated);
    return updated;
  }

  async deleteResidence(_id: string): Promise<boolean> {
    return this.residences.delete(_id);
  }

  // Document operations - required by IStorage interface
  ...
  async createBuildingDocument(_document: InsertDocumentBuilding): Promise<DocumentBuilding> {
    const id = randomUUID();
    const document: DocumentBuilding = {
      ..._document,
      id,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  async deleteBuildingDocument(_id: string): Promise<boolean> {
    return false;
  }
  ...
  // Password reset operations - required by IStorage interface
