...
async function registerRoutes(app2) {
  ...
  app2.get("/", (req, res) => {
    try {
      ...
      const indexPath = isDev ? path4.resolve(process.cwd(), "client/index.html") : path4.resolve(process.cwd(), "dist/public/index.html");
    ...
    server = app.listen(port, "0.0.0.0", () => {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        setupStaticServing().then(() => {
          ...
          log(`\u26A0\uFE0F Failed to set up static serving: ${error2.message}`, "error");
        });
        setTimeout(() => {
          loadFullApplication().catch((error2) => {
            log(`\u26A0\uFE0F Full application load failed: ${error2.message}`, "error");
...
async function setupStaticServing() {
  try {
    ...
    throw new Error(`Failed to set up static serving: ${error2.message}`);
  }
}
