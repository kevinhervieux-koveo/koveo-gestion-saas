  ...
  try {
    ...
    if (orphanedUserRes.length > 0) {
      for (const orphan of orphanedUserRes) {
        await db
          .delete(userResidences)
          .where(eq(userResidences.id, orphan.user_residences.id));
      }
    ...
    if (orphanedBuildings.length > 0) {
      ...
      for (const building of orphanedBuildings) {
        await db
          .delete(buildings)
          .where(eq(buildings.id, building.buildings.id));
      }
    ...
    if (orphanedResidences.length > 0) {
      ...
      for (const residence of orphanedResidences) {
        await db
          .delete(residences)
          .where(eq(residences.id, residence.residences.id));
      }
    ...
    if (orphanedNotifications.length > 0) {
      for (const notification of orphanedNotifications) {
        await db
          ...
          .where(eq(notifications.id, notification.notifications.id));
      }
    ...
    const verificationResults = await Promise.all([
      db.select().from(userOrganizations).leftJoin(users, eq(userOrganizations.userId, users.id)).where(isNull(users.id)),
      db.select().from(userResidences).leftJoin(users, eq(userResidences.userId, users.id)).where(isNull(users.id)),
      db.select().from(buildings).leftJoin(organizations, eq(buildings.organizationId, organizations.id)).where(isNull(organizations.id)),
      db.select().from(residences).leftJoin(buildings, eq(residences.buildingId, buildings.id)).where(isNull(buildings.id)),
      db.select().from(notifications).leftJoin(users, eq(notifications.userId, users.id)).where(isNull(users.id))
    ]);
