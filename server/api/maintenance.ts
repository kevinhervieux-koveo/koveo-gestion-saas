    ...
    try {
      ...
      const elementsNeedingMaintenance = await db
        ...
        .where(and(
          eq(buildingElements.buildingId, buildingId),
          eq(buildingElements.isActive, true),
          inArray(buildingElements.currentCondition, ['poor', 'critical']),
      ...
      for (const element of elementsNeedingMaintenance) {
        try {
          ...
          const suggestedType = element.currentCondition === 'critical' ? 'replacement' : 'major_rehab';
          const priority = element.currentCondition === 'critical' ? 'critical' : 'high';
          
          // Generate project title and description
          const title = `${suggestedType === 'replacement' ? 'Replace' : 'Rehabilitate'} ${element.name}`;
          const description = `Auto-generated project for ${element.name} (${element.uniformatCode}) due to ${element.currentCondition} condition. Immediate attention required.`;
