...
export function registerBillRoutes(app: Express) {
  ...
  // New endpoint for creating bills from auto-generated templates with file support
  app.post('/api/bills/from-auto-generated', 
    ...
    try {
      ...
      let parsedBillData;
      try {
        parsedBillData = typeof newBillData === 'string' ? JSON.parse(newBillData) : newBillData;
        
        if (!parsedBillData || typeof parsedBillData !== 'object') {
          throw new Error('Bill data must be an object');
        }
      } catch (error) {
        return res.status(400).json({
          message: 'Invalid bill data format - must be valid JSON object',
          error: 'INVALID_BILL_DATA',
          code: 'VALIDATION_ERROR'
        });
      }

      // Validate required fields
      const requiredFields = ['title', 'category', 'totalAmount'];
      const missingFields = requiredFields.filter(field => !parsedBillData[field]);
      ...
      // Create the new bill using the service with retry logic for duplicate bill numbers
      let newBill: Bill;
      let attempts = 0;
      const maxAttempts = 3;
      ...
      while (attempts < maxAttempts) {
        try {
          newBill = await billAutoGenerationService.createAutoGeneratedBill(
            template,
            {
              ...parsedBillData,
              createdBy: req.user.id,
          ...
          attempts++;
          if (error.message?.includes('Failed to generate unique bill number') && attempts < maxAttempts) {
            console.warn(`🔄 Retrying bill creation due to bill number generation failure (attempt ${attempts}/${maxAttempts})`);
            await new Promise(resolve => setTimeout(resolve, 100 * attempts)); // Progressive delay
            continue;
          }
          throw error; // Re-throw if not a bill number issue or max attempts reached
        }
      }

      // Insert the bill into database with transaction
      let insertResult;
      try {
        insertResult = await db.insert(bills).values({
          ...newBill!,
          id: undefined, // Let database generate ID
        }).returning();
        
        insertedBill = insertResult[0];
        console.log(`✅ Created bill ${insertedBill.id} with number ${insertedBill.billNumber}`);
      } catch (dbError: any) {
        if (dbError.message?.includes('duplicate key') || dbError.message?.includes('bill_number')) {
          return res.status(409).json({
            message: 'Bill number already exists. Please try again.',
            error: 'DUPLICATE_BILL_NUMBER',
            code: 'CONFLICT_ERROR',
      ...
      // Process and save uploaded files with proper error handling
      if (uploadedFiles && uploadedFiles.length > 0) {
        console.log(`📎 Processing ${uploadedFiles.length} files for bill ${insertedBill.id}`);
        ...
        for (let i = 0; i < uploadedFiles.length; i++) {
          ...
          try {
            ...
            if (req.body[metadataKey]) {
              fileMetadata = JSON.parse(req.body[metadataKey]);
            }
          } catch (error) {
            console.warn(`⚠️ Failed to parse metadata for file ${i}:`, error);
          }
          ...
          try {
            ...
            if (file.size > 10 * 1024 * 1024) { // 10MB limit
              throw new Error(`File ${file.originalname} exceeds size limit of 10MB`);
            }

            // Create upload context for bills
            const uploadContext: UploadContext = {
              type: 'bills',
              organizationId: req.user.organizationId,
              buildingId: insertedBill.buildingId,
              userId: req.user.id,
              userRole: req.user.role
            };

            // Save file using secure storage with error handling
            const saveResult = await secureFileStorage.storeFile(
              file,
              uploadContext,
              req.user.role,
              req.user.id
            ...
            if (saveResult.success && saveResult.filePath) {
              // Create document record with proper validation
              const documentData = {
                name: file.originalname,
                description: `Document attached to bill: ${insertedBill.title}`,
                documentType: (fileMetadata as any)?.category || 'attachment',
                ...
                buildingId: insertedBill.buildingId,
                uploadedById: req.user.id,
                attachedToType: 'bill' as const,
                attachedToId: insertedBill.id,
              };

              const insertedDocuments = await db.insert(documents).values(documentData).returning();
              savedDocuments.push(insertedDocuments[0]);
              
              console.log(`✅ Saved document: ${file.originalname} for bill ${insertedBill.id}`);
            } else {
              console.error(`❌ Failed to save file: ${file.originalname}`, saveResult.error);
              throw new Error(`Failed to save file ${file.originalname}: ${saveResult.error}`);
            }
          } catch (fileError: any) {
            console.error(`❌ Error processing file ${file.originalname}:`, fileError);
            // For critical file processing errors, we should fail the entire operation
            throw new Error(`File processing failed for ${file.originalname}: ${fileError.message}`);
          }
        }
      }

      // Success response with complete information
      res.status(201).json({
        success: true,
        message: 'Bill created successfully from auto-generated template',
        bill: insertedBill,
        attachedDocuments: savedDocuments,
        documentsCount: savedDocuments.length,
        updatedSourceBill: template.sourceTemplateId ? true : false,
        metadata: {
          templateId,
          createdAt: new Date().toISOString(),
          filesProcessed: uploadedFiles?.length || 0
        }
      });
      ...
      // Cleanup on failure - remove the bill if it was created
      if (insertedBill?.id) {
        try {
          await db.delete(bills).where(eq(bills.id, insertedBill.id));
          await db.delete(documents).where(eq(documents.attachedToId, insertedBill.id));
          console.log(`🧹 Cleaned up failed bill creation: ${insertedBill.id}`);
        } catch (cleanupError) {
      ...
      for (const tempFile of tempFiles) {
        try {
          ...
          console.warn(`⚠️ Could not cleanup temp file ${tempFile}:`, cleanupError);
        }
      }

      // Return appropriate error response based on error type
      if (error.message?.includes('Template bill not found')) {
        return res.status(404).json({
          success: false,
          message: 'Template bill not found',
          error: 'TEMPLATE_NOT_FOUND',
      ...
      } else if (error.message?.includes('Bill number already exists') || error.message?.includes('duplicate key')) {
        return res.status(409).json({
          success: false,
    ...
    async (req: any, res: any) => {
      ...
      try {
        ...
        const newBillData = {
          ...
          notes: validatedBillData.notes || null,
        ...
        // Generate payments for the bill
        try {
          await paymentGenerationService.generatePaymentsForBill(createdBill.id);
          console.log(`[AUTO-GENERATED BILL] Payments generated for bill ${createdBill.id}`);
        } catch (paymentError) {
          console.warn(`[AUTO-GENERATED BILL] Failed to generate payments:`, paymentError);
          // Don't fail the bill creation if payment generation fails
        }
        ...
        if (createdBill.paymentType === 'recurrent') {
          try {
            await billAutoGenerationService.generateNextYearBill(createdBill);
            await billAutoGenerationService.cleanupPastAutoGeneratedBills(createdBill.buildingId);
