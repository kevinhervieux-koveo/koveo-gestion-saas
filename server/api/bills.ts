...
export function registerBillRoutes(app: Express) {
  ...
  app.get('/api/bills', requireAuth, async (req: any, res: any) => {
    try {
      const { buildingId, category, year, status = 'all', months, paymentType, isAutoGenerated } = req.query;
  ...
  app.get('/api/bills/generation-stats', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      let autoGeneratedQuery = db
        .select({ count: sql<number>`count(*)` })
        .from(bills)
        .where(eq(bills.isAutoGenerated, true));

      if (buildingId) {
        recurrentQuery = recurrentQuery.where(eq(bills.buildingId, buildingId as string));
        autoGeneratedQuery = autoGeneratedQuery.where(eq(bills.buildingId, buildingId as string));
      } else if (organizationId) {
        const orgCondition = sql`${bills.buildingId} IN (
          SELECT id FROM ${buildings} WHERE organization_id = ${organizationId}
        )`;
        recurrentQuery = recurrentQuery.where(orgCondition);
        autoGeneratedQuery = autoGeneratedQuery.where(orgCondition);
      }
  ...
  app.post('/api/bills/auto-generate', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const result = await billAutoGenerationService.generateBillFromTemplate({
        sourceTemplate: template,
        targetDate: targetDate ? new Date(targetDate) : new Date(),
        periodLabel: generateForPeriod || 'Manual Generation'
      });
