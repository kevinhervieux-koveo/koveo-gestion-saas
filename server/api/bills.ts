...
export function registerBillRoutes(app: Express) {
  ...
  app.patch('/api/bills/:id', requireAuth, async (req: any, res: any) => {
    try {
      ...
      if (updatedBill.paymentType === 'recurrent') {
        try {
          // Auto-generated bill regeneration handled by financial service
          // Cleanup handled by financial service internally
  ...
  app.post('/api/bills/bulk-generate', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const result = await financialService.createAutoGeneratedBill({
        organizationId,
        buildingIds,
        forceRegenerate
      });
  ...
  app.post('/api/bills/run-scheduled-job', requireAuth, async (req: Request, res: Response) => {
    try {
      const result = { message: 'Scheduled generation handled by financial service automatically' };
