...
import { resolveOrgScope, assertBuildingWriteAccess, assertBillWriteAccess } from '../utils/org-scope';
const { buildings, bills, documents, payments, userBuildings, residences, userResidences } = schema;

/**
 * Mint a bill number for an API-created bill. Uses the unified V2 format
 * when BILL_NUMBER_V2 is enabled, otherwise falls back to the legacy
 * BILL-{timestamp}-{shortUuid} format. Designed to be called inside
 * withBillNumberRetry — invoke it fresh on every retry.
 */
async function mintApiBillNumber(opts: {
  buildingId: string;
  billingPeriod: Date | string;
  category: string | null | undefined;
}): Promise<string> {
  if (isBillNumberV2Enabled()) {
    const orgCode = await resolveOrgCodeForBuilding(opts.buildingId);
...
const createBillSchema = z.object({
  ...
  // AI-extracted vendor identifiers (Task #338). Both nullable + optional so
  // manual bill creation flows that don't surface these fields keep working.
  issueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Issue date must be YYYY-MM-DD').nullable().optional(),
...
export function registerBillRoutes(app: import('../utils/lazy-mount').RouteRegistry) {
  ...
  app.get('/api/bills', requireAuth, async (req: any, res: any) => {
    try {
      const { buildingId, category, year, status = 'all', months, paymentType, billType, paymentStructure, vendor, isAutoGenerated, search, issueDateFrom, issueDateTo } = req.query;

      logDebug('[BILLS API] Fetching bills with params', { metadata: { buildingId, category, year, status, months, paymentType, billType, paymentStructure, vendor, isAutoGenerated, search, issueDateFrom, issueDateTo } });
      ...
      // Restrict bills to buildings owned by the resolved org set. Empty set
      // means the caller has no accessible orgs — return no bills rather than
      // the full cross-organization dataset.
      ...
      const conditions = [inArray(bills.buildingId, scopedBuildingIds)];
      ...
      if (search && search.trim()) {
        ...
        const searchConditions = or(
          ...
          ilike(bills.vendorInvoiceNumber, searchTerm),
          ilike(bills.billNumber, searchTerm),
  ...
  app.post('/api/bills', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: verify the caller has write
      // access to the target building's organization before minting a
      // bill number against it. Without this guard a manager attached
      // to org-A could create bills against any building id they
      // happen to know — including buildings in unrelated tenants.
      const buildingAccess = await assertBuildingWriteAccess(res, req.user.id, billData.buildingId);
      if (!buildingAccess.ok) {
        return;
      }
      ...
      const newBill = await withBillNumberRetry(
        () =>
          mintApiBillNumber({
            buildingId: billData.buildingId,
            billingPeriod: billData.startDate,
        ...
        async (billNumber) => {
          const newBillData = {
            ...
            issueDate: billData.issueDate || null,
            vendorInvoiceNumber: billData.vendorInvoiceNumber || null,
          ...
          const rows = await db.insert(bills).values(newBillData as any).returning();
          return rows as any[];
        },
      );
  ...
  app.patch('/api/bills/:id', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the update unless the
      // caller can access the bill's building organization.
      const patchAccess = await assertBuildingWriteAccess(
        res,
        req.user.id,
        originalBillRecord.buildingId,
      );
      if (!patchAccess.ok) {
        return;
      }
      ...
      // AI-extracted vendor identifiers (Task #338)
      if ('issueDate' in req.body) {
        updateData.issueDate = billData.issueDate || null;
      }
      if ('vendorInvoiceNumber' in req.body) {
        updateData.vendorInvoiceNumber = billData.vendorInvoiceNumber || null;
      }
  ...
  app.put('/api/bills/:id', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the update unless the
      // caller can access the bill's building organization.
      const putAccess = await assertBuildingWriteAccess(
        res,
        req.user.id,
        originalBillRecord2.buildingId,
      );
      if (!putAccess.ok) {
        return;
      }
    ...
    try {
      ...
      if (shouldHaveAutoGenerated) {
        ...
        if (generatedBills.length > 0) {
          ...
          if (hasAutoGenerated) {
            // Update existing — preserve the existing bill's bill_number (per task #255:
            // historical bill numbers must NEVER be renamed). Only sync the mutable
            // fields onto the row.
            ...
            // Create new — re-mint a unified V2 bill number per attempt and rely on
            // the UNIQUE constraint + retry to absorb sequence races.
            const { id, createdAt, updatedAt, billNumber: legacyNumber, ...insertData } = autoGenBillData as any;
            const startDateForMint = autoGenBillData.startDate as unknown as string;
            await withBillNumberRetry(
              async () => {
                if (isBillNumberV2Enabled()) {
                  ...
                  return generateBillNumberV2({
                    orgCode,
                    billingPeriod: startDateForMint,
                    category: autoGenBillData.category,
                  });
                }
                return legacyNumber;
  ...
  app.delete('/api/bills/:id', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the delete unless the
      // caller can access the bill's building organization.
      const deleteAccess = await assertBuildingWriteAccess(
        res,
        req.user.id,
        billToDelete[0].buildingId,
      );
      if (!deleteAccess.ok) {
        return;
      }
  ...
  app.post(
    ...
    async (req: any, res: any) => {
      ...
      try {
        ...
        // Task #1271 — Org-scope hardening: refuse the upload unless
        // the caller can access the bill's building organization. We
        // also clean up the temp file so we don't leak disk on a 403.
        const uploadAccess = await assertBuildingWriteAccess(res, userId, bill.buildingId);
        if (!uploadAccess.ok) {
          if (fs.existsSync(req.file.path)) {
            fs.unlinkSync(req.file.path);
          }
  ...
  app.delete('/api/bills/:id/attachment', requireAuth, async (req: any, res: any) => {
    ...
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the attachment delete
      // unless the caller can access the bill's building organization.
      const attachmentDeleteAccess = await assertBuildingWriteAccess(
        res,
        req.user.id,
        billData.buildingId,
      );
      if (!attachmentDeleteAccess.ok) {
        return;
      }
  ...
  app.post('/api/bills/:id/apply-ai-analysis', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the AI re-application
      // unless the caller can access the bill's building organization.
      const aiApplyAccess = await assertBuildingWriteAccess(
        res,
        req.user.id,
        billData.buildingId,
      );
  ...
  app.post('/api/bills/:id/generate-future', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the generation unless
      // the caller can access the bill's building organization.
      // Replaces a previous "building exists" check that did not verify
      // the caller's org membership.
      const generateFutureAccess = await assertBuildingWriteAccess(
        res,
        req.user.id,
        bill[0].buildingId,
      );
      if (!generateFutureAccess.ok) {
        return;
      }
  ...
  app.post('/api/bills/:id/generate-next-year', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse generation unless the
      // caller can access the bill's building organization.
      const generateNextYearAccess = await assertBuildingWriteAccess(
        res,
        (req as any).user.id,
        bill.buildingId,
      );
      if (!generateNextYearAccess.ok) {
        return;
      }
      ...
      // Insert generated bills into database — re-mint a unified V2 bill number
      // per attempt so unique-constraint races just retry instead of failing.
      const insertedBills = [];
      const generatedList = Array.isArray(generatedBills) ? generatedBills : [generatedBills];
      for (const generatedBill of generatedList) {
        const { id: _omitId, billNumber: legacyNumber, ...billRest } = generatedBill as any;
        const insertResult = await withBillNumberRetry(
          async () => {
            if (isBillNumberV2Enabled()) {
              const orgCode = await resolveOrgCodeForBuilding((generatedBill as any).buildingId);
              return generateBillNumberV2({
                orgCode,
                billingPeriod: (generatedBill as any).startDate,
                category: (generatedBill as any).category,
              });
            }
            return legacyNumber;
          },
  ...
  app.post('/api/bills/from-template', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the create unless the
      // caller can access the target building's organization.
      const fromTemplateAccess = await assertBuildingWriteAccess(
        res,
        (req as any).user.id,
        validatedData.buildingId,
      );
      if (!fromTemplateAccess.ok) {
        return;
      }
      ...
      const insertResult = await withBillNumberRetry(
        () =>
          mintApiBillNumber({
            ...
            billingPeriod: validatedData.startDate,
        ...
        async (billNumber) => {
          ...
          const rows = await db.insert(bills).values(billFromTemplate as any).returning();
          return rows as any[];
        },
      );
  ...
  app.post('/api/bills/bulk-generate', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: bulk generation is the
      // highest-blast-radius write in this file. Without an explicit
      // org-scoped gate the underlying service fans out across every
      // building (or every building id supplied by the caller) which
      // would silently let any authenticated user trigger writes
      // across other tenants. Resolution rules:
      //   1. organizationId is REQUIRED. Calls without it are
      //      rejected so the service never receives `undefined`,
      //      which today expands to "all buildings".
      //   2. The caller must belong to the requested organization.
      //   3. If buildingIds is supplied, every entry must resolve to
      //      an active building inside that organization. Any
      //      foreign id rejects the entire request — partial
      //      acceptance would leak existence information about
      ...
      const accessibleOrgIds = await getUserAccessibleOrganizations(callerId);
      ...
      let scopedBuildingIds: string[] | undefined = undefined;
      if (Array.isArray(buildingIds) && buildingIds.length > 0) {
        const requestedIds = buildingIds.filter((b: any) => typeof b === 'string');
        if (requestedIds.length !== buildingIds.length) {
          return res.status(400).json({
            message: 'buildingIds must be an array of strings',
            code: 'INVALID_BUILDING_IDS',
          });
        }
        const matchingBuildings = await db
          ...
          .where(
            and(
              ...
              inArray(schema.buildings.id, requestedIds),
            ),
          );
        if (matchingBuildings.length !== requestedIds.length) {
          return res.status(403).json({
            message: 'One or more buildingIds do not belong to this organization',
            code: 'INSUFFICIENT_PERMISSIONS',
          });
        }
        scopedBuildingIds = matchingBuildings.map((b: any) => b.id);
      }
  ...
  app.post('/api/bills/auto-generate', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the auto-generation
      // unless the caller can access the source template's building.
      const autoGenAccess = await assertBuildingWriteAccess(
        res,
        (req as any).user.id,
        template.buildingId,
      );
      ...
      // Prepare the bill for insertion by omitting auto-generated fields. Re-mint
      // a unified V2 bill number per attempt and rely on the UNIQUE constraint
      // + retry to absorb sequence races.
      const firstGenerated = (generatedBills as any[])[0];
      const { id: _omitId, createdAt: _omitCreated, updatedAt: _omitUpdated, billNumber: legacyNumber, ...billRest } = firstGenerated;
      const insertResult = await withBillNumberRetry(
        async () => {
          if (isBillNumberV2Enabled()) {
            const orgCode = await resolveOrgCodeForBuilding(firstGenerated.buildingId);
            return generateBillNumberV2({
              orgCode,
              billingPeriod: firstGenerated.startDate,
              category: firstGenerated.category,
            });
          }
          return legacyNumber;
        },
      ...
      const result = { bill: (insertResult as any[])[0] };
  ...
  app.patch('/api/bills/:billId/payments/:paymentId', requireAuth, async (req: any, res: any) => {
    try {
      ...
      // Task #1271 — Org-scope hardening: refuse the per-payment update
      // unless the caller can access the bill's building organization.
      // Loaded via assertBillWriteAccess so the route returns 404 for
      // unknown bill ids and 403 for cross-org bills.
      const paymentAccess = await assertBillWriteAccess(res, req.user.id, billId);
      if (!paymentAccess.ok) {
        return;
      }

      // Verify the payment actually belongs to this bill — otherwise the
      // org check above could be bypassed by passing a bill id you can
      // see plus a payment id from another bill in another org.
      const paymentCheckRows = await db
        .select({ id: payments.id, billId: payments.billId })
        .from(payments)
        .where(eq(payments.id, paymentId))
        .limit(1);
      if (paymentCheckRows.length === 0 || paymentCheckRows[0].billId !== billId) {
        return res.status(404).json({
  ...
  app.post('/api/bills/from-auto-generated', 
    ...
    async (req: any, res: any) => {
      ...
      try {
        ...
        // Task #1271 — Org-scope hardening: refuse the create unless
        // the caller can access the resolved building's organization.
        // Without this, anyone who can guess a templateId from another
        // tenant could create bills (and attach files) against that
        // tenant's building.
        const fromAutoAccess = await assertBuildingWriteAccess(res, userId, buildingId);
        if (!fromAutoAccess.ok) {
          return;
        }

        // Create the bill (with V2 number when the flag is on)
        const newBill = await withBillNumberRetry(
          () =>
            mintApiBillNumber({
              buildingId,
              billingPeriod: validatedBillData.startDate,
          ...
          async (billNumber) => {
            ...
            const rows = await db.insert(bills).values([newBillData]).returning();
            return rows as any[];
          },
        );
