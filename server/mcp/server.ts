...
import { registerDocumentTagMcpTools } from "./document-tag-mcp-tools";
...
import { DEMAND_COMMENT_TEXT_MAX, insertDemandCommentSchema, MAINTENANCE_CATEGORY_VALUES } from "@shared/schemas/operations";
...
export function createMcpServer(authContext?: McpAuthContext): McpServer {
  ...
  server.tool(
    ...
    async ({ role, buildingId, status }) => {
      ...
      try {
        ...
        // Task #1276: select an EXPLICIT column list rather than the implicit
        // `db.select()` that fans out to `SELECT *`. The implicit form caused
        // task #652 to crash every `list_bills` call when a column declared
        // in `shared/schemas/financial.ts` (e.g. `original_file_name`) was
        // missing from the deployed database — a class of failure we never
        // want to repeat. The list below is intentionally limited to the
        // fields useful for an MCP caller building a bills overview;
        // intentionally OMITTED here:
        //   - `aiAnalysisData` (jsonb): potentially large per-row payload
        //     of AI-extracted fields; callers that actually need it should
        //     fetch a single row via `get_bill` instead of paying the cost
        //     on every list call.
        //   - `notes`: free-form internal text not useful for list overviews.
        //   - `parentBillId` / `sourceTemplateId` / `autoGeneratedLabel`:
        //     internal bookkeeping for the auto-generation pipeline.
        //   - `source`: internal "where did this row come from" tag.
        // Add new fields here only when they are part of the overview
        // contract — anything heavier or AI-specific belongs in `get_bill`.
        const bills = await db
          .select({
            ...
            paymentStructure: schema.bills.paymentStructure,
            paymentType: schema.bills.paymentType,
            issueDate: schema.bills.issueDate,
            vendorInvoiceNumber: schema.bills.vendorInvoiceNumber,
            schedulePayment: schema.bills.schedulePayment,
            yearInterval: schema.bills.yearInterval,
            scheduleCustom: schema.bills.scheduleCustom,
            costs: schema.bills.costs,
            totalAmount: schema.bills.totalAmount,
            startDate: schema.bills.startDate,
            endDate: schema.bills.endDate,
            ...
            autoGenerateNextYear: schema.bills.autoGenerateNextYear,
  ...
  server.tool(
    ...
    "Add a comment to a demand. Tenants may only comment on their own demands and cannot post internal (manager-only) comments. Triggers the demand-commented notification flow. Optionally attach a single file by passing an `attachment` (e.g. an object-storage URL the assistant just uploaded); the ACL check + binding mirrors the demand attachment flow.",
    {
      role: roleParam,
      demandId: z.string().describe("Demand ID"),
      // Mirror the cap from insertDemandCommentSchema (DEMAND_COMMENT_TEXT_MAX)
      // here so oversized bodies are rejected at the MCP boundary instead of
      // bouncing off the inner safeParse with a generic "Invalid comment data"
      // error — and so a future regression that drops the inner validator
      ...
      commentText: z
        ...
        .max(
          ...
          `Comment must not exceed ${DEMAND_COMMENT_TEXT_MAX} characters`,
        )
        .describe(`Comment body (1-${DEMAND_COMMENT_TEXT_MAX} characters)`),
  ...
  server.tool(
    ...
    async ({ role, invitationId }) => {
      ...
      // residence guard above: `invitations.buildingId` carries no
      // DB-level FK, so a hard-deleted building (or any row that
      // slipped through `delete_building`'s soft-cancel sweep) can
      // leave the invitation pointing at a building that no longer
      // exists. Resending it would email a token that, on acceptance,
      // would fail to hydrate the building link. Refuse to resurrect
      // the row; operators should cancel the invitation or re-invite
      ...
      if (invitation.buildingId) {
        ...
        if (!buildingRow) {
          return {
            content: [{
              type: "text" as const,
              text: `Building not found: ${invitation.buildingId} (cannot resend invitation pointing at a deleted building)`,
            }],
          };
        }
      }
  ...
  registerDocumentTagMcpTools(server, { roleParam, getMcpOrgIds });
