...
const detectFormattingPatterns = (
  content: string
): {
  cssClasses: string[];
  inlineStyles: string[];
...
describe('Code Redundancy Detection Tests', () => {
  ...
  beforeAll(() => {
    ...
    allContent = sourceFiles.map((file) => getFileContent(file)).join('\n');
  });
    ...
    it('should analyze form validation redundancy', () => {
      ...
      sourceFiles.forEach((file) => {
        ...
        const validationRules =
          content.match(/\.required\(\)|\.min\(|\.max\(|\.email\(\)|\.phone\(\)/g) || [];
  ...
  describe('Button Component Redundancy Analysis', () => {
    ...
    it('should analyze button action patterns', () => {
      ...
      actionPatterns.forEach((action) => {
        if (action.includes('handleSubmit')) {
          actionTypes.set('submit', (actionTypes.get('submit') || 0) + 1);
        }
        if (action.includes('handleCancel')) {
          actionTypes.set('cancel', (actionTypes.get('cancel') || 0) + 1);
        }
        if (action.includes('handleDelete')) {
          actionTypes.set('delete', (actionTypes.get('delete') || 0) + 1);
        }
        if (action.includes('handleEdit')) {
          actionTypes.set('edit', (actionTypes.get('edit') || 0) + 1);
        }
        if (action.includes('handleCreate')) {
          actionTypes.set('create', (actionTypes.get('create') || 0) + 1);
        }
      });
  ...
  describe('Card Component Redundancy Analysis', () => {
    ...
    it('should analyze card content structure patterns', () => {
      ...
      sourceFiles.forEach((file) => {
        ...
        const structures =
          content.match(
            /<div[^>]*card-header[^>]*>|<div[^>]*card-body[^>]*>|<div[^>]*card-footer[^>]*>/g
          ) || [];
  ...
  describe('CSS and Formatting Redundancy Analysis', () => {
    it('should identify duplicate styling patterns', () => {
      ...
      console.warn(
        `- Total typography patterns: ${allFormattingPatterns.typographyPatterns.length}`
      );
    ...
    it('should identify most commonly used style combinations', () => {
      ...
      sourceFiles.forEach((file) => {
        ...
        styleCombinatiors.push(
          ...classMatches.map((match) => match.replace(/className=["']([^"']*)["']/, '$1').trim())
        );
      });
    ...
    it('should prioritize refactoring efforts by impact', () => {
      ...
      refactoringPriorities
        .sort((a, b) => {
          ...
          const aScore =
            impactScore[a.impact as keyof typeof impactScore] +
            effortScore[a.effort as keyof typeof effortScore];
          const bScore =
            impactScore[b.impact as keyof typeof impactScore] +
        ...
        .forEach((priority, _index) => {
          console.warn(
            `${index + 1}. ${priority.area} (Impact: ${priority.impact}, Effort: ${priority.effort})`
          );
