...
const categorizeStyle = (className: string): StylePattern['category'] => {
  if (
    className.match(
      /^(bg-|text-|border-|ring-|decoration-|divide-).*?-(red|blue|green|yellow|purple|pink|indigo|gray|black|white|orange|teal|cyan|lime|emerald|sky|violet|fuchsia|rose)-\d+$/
    )
  ) {
    return 'color';
  }
  if (
    className.match(/^(p-|m-|px-|py-|mx-|my-|pt-|pb-|pl-|pr-|mt-|mb-|ml-|mr-|gap-|space-[xy]-)\d+$/)
  ) {
    return 'spacing';
  }
  ...
  if (
    className.match(
      /^(flex|grid|block|inline|absolute|relative|fixed|sticky|top-|left-|right-|bottom-|w-|h-|max-|min-)/
    )
  ) {
    return 'layout';
  }
...
const generateDesignTokens = (stylePatterns: StylePattern[]): DesignToken[] => {
  ...
  colorPatterns.forEach((pattern) => {
    const colorMatch = pattern.pattern.match(
      /(red|blue|green|yellow|purple|pink|indigo|gray|black|white|orange|teal|cyan|lime|emerald|sky|violet|fuchsia|rose)-(\d+)/
    );
    if (colorMatch) {
  ...
  colorMap.forEach((data, tokenName) => {
    if (data.usage >= 3) {
      // Only create tokens for frequently used colors
      tokens.push({
        name: tokenName,
        _value: data.value,
        category: 'colors',
  ...
  spacingMap.forEach((data, size) => {
    if (data.usage >= 5) {
      // Only create tokens for frequently used spacing
      tokens.push({
        name: `spacing-${size}`,
...
describe('Style Consolidation Analysis Tests', () => {
      ...
      try {
        ...
        for (const item of items) {
          ...
          if (item.isDirectory() && !item.name.startsWith('.') && item.name !== 'node_modules') {
            ...
            item.isFile() &&
            (item.name.endsWith('.tsx') || item.name.endsWith('.ts') || item.name.endsWith('.css'))
          ) {
  ...
  describe('CSS Class Usage Analysis', () => {
    it('should analyze CSS class frequency and patterns', () => {
      ...
      classFrequency.forEach((count, className) => {
        if (count > 1) {
          // Only consider classes used more than once
          stylePatterns.push({
            pattern: className,
            occurrences: count,
      ...
      const topClasses = stylePatterns.sort((a, b) => b.occurrences - a.occurrences).slice(0, 10);

      console.warn('\nMost frequently used classes:');
      topClasses.forEach((pattern, _index) => {
        console.warn(
          `${index + 1}. ${pattern.pattern}: ${pattern.occurrences} times (${pattern.category})`
        );
      });
    ...
    it('should identify color usage patterns', () => {
      const colorPatterns = allStyles.filter((className) => {
        return className.match(
          /(bg-|text-|border-).*?-(red|blue|green|yellow|purple|pink|indigo|gray|black|white|orange|teal|cyan|lime|emerald|sky|violet|fuchsia|rose)-\d+/
        );
      });
      ...
      const topColors = [...colorUsage.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
    ...
    it('should analyze spacing consistency', () => {
      const spacingPatterns = allStyles.filter((className) => {
        return className.match(
          /^(p-|m-|px-|py-|mx-|my-|pt-|pb-|pl-|pr-|mt-|mb-|ml-|mr-|gap-|space-[xy]-)\d+$/
        );
      });
      ...
      const topSpacing = [...spacingUsage.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8);
  ...
  describe('Design Token Generation', () => {
    it('should generate comprehensive design token system', () => {
      ...
      const tokensByCategory = designTokens.reduce(
        (acc, token) => {
      ...
      Object.entries(tokensByCategory).forEach(([category, tokens]) => {
        ...
        tokens.forEach((token) => {
          console.warn(
            `--${token.name}: ${token.value}; /* Used ${token.usage} times across ${token.files.length} files */`
          );
        });
        console.warn('');
      });
  ...
  describe('Utility Class Consolidation', () => {
    ...
    it('should calculate consolidation impact', () => {
      ...
      console.warn(
        `Total class combinations found: ${consolidationMetrics.totalClassCombinations}`
      );
      console.warn(`Duplicate combinations: ${consolidationMetrics.duplicateCombinations}`);
      console.warn(
        `Consolidation opportunities: ${consolidationMetrics.consolidationOpportunities}`
      );
      console.warn(
        `Estimated character savings: ${consolidationMetrics.estimatedCharacterSavings}`
      );
      ...
      const consolidationRatio =
        (consolidationMetrics.duplicateCombinations / consolidationMetrics.totalClassCombinations) *
        100;
      ...
      const migrationSteps = [
        {
          ...
          commands: ['npm run analyze-styles', 'npm run generate-design-tokens'],
        },
        {
