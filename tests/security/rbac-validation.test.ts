  ...
  describe('Role Hierarchy Validation', () => {
    ...
    it('should ensure admin has all critical system permissions', () => {
      const criticalPermissions: Permission[] = [
        ...
        'delete:building', 'delete:residence',
        'read:audit_log', 'update:framework_config'
      ];

      criticalPermissions.forEach(permission => {
        expect(checkPermission(permissions, 'admin', permission))
    ...
    it('should verify managers have appropriate property management permissions', () => {
      const managerPermissions: Permission[] = [
        'read:building', 'create:building', 'update:building',
        'read:residence', 'create:residence', 'update:residence',
        'read:bill', 'create:bill', 'update:bill', 'approve:bill',
        'read:maintenance_request', 'assign:maintenance_request'
      ];

      managerPermissions.forEach(permission => {
        expect(checkPermission(permissions, 'manager', permission))
    ...
    it('should ensure tenant/resident permissions are appropriately restricted', () => {
      const restrictedPermissions: Permission[] = [
        'delete:user', 'create:organization', 'delete:building',
        'delete:residence', 'approve:bill', 'delete:maintenance_request',
        'read:audit_log', 'update:framework_config'
      ];

      restrictedPermissions.forEach(permission => {
        expect(checkPermission(permissions, 'tenant', permission))
          .toBe(false, `Tenant should not have permission: ${permission}`);
        expect(checkPermission(permissions, 'resident', permission))
  ...
  describe('Permission Categories Validation', () => {
    it('should validate user management permissions across roles', () => {
      ...
      userPermissions.forEach(permission => {
        expect(checkPermission(permissions, 'admin', permission as Permission))
          .toBe(true, `Admin should have ${permission}`);
      });
    ...
    it('should validate building management permissions', () => {
      const buildingPermissions = ['read:building', 'create:building', 'update:building', 'delete:building'];
    ...
    it('should validate financial management permissions', () => {
      const financialPermissions = [
        'read:bill', 'create:bill', 'update:bill', 'delete:bill', 'approve:bill',
        'read:budget', 'create:budget', 'update:budget', 'delete:budget'
      ];
    ...
    it('should validate maintenance request permissions', () => {
      const maintenancePermissions = [
        'read:maintenance_request', 'create:maintenance_request', 
        'update:maintenance_request', 'update:own_maintenance_request',
        'delete:maintenance_request', 'assign:maintenance_request'
      ];
  ...
  describe('Permission Boundary Testing', () => {
    it('should prevent privilege escalation attempts', () => {
      ...
      privilegeEscalationTests.forEach(({ role, permission }) => {
        expect(checkPermission(permissions, role as Role, permission as Permission))
    ...
    it('should enforce strict boundaries for system administration', () => {
      const systemAdminPermissions: Permission[] = [
        'read:audit_log', 'update:framework_config', 'delete:improvement_suggestion',
        'delete:feature', 'delete:actionable_item', 'backup:system' as Permission
      ];

      // Only admin should have system administration permissions
      systemAdminPermissions.forEach(permission => {
        expect(checkPermission(permissions, 'admin', permission))
          .toBe(true, `Admin should have system permission: ${permission}`);
        expect(checkPermission(permissions, 'manager', permission))
          .toBe(false, `Manager should not have system permission: ${permission}`);
        expect(checkPermission(permissions, 'tenant', permission))
          .toBe(false, `Tenant should not have system permission: ${permission}`);
        expect(checkPermission(permissions, 'resident', permission))
    ...
    it('should validate Quebec Law 25 compliance permissions', () => {
      ...
      privacyPermissions.forEach(permission => {
        const hasPermission = (role: Role) => checkPermission(permissions, role, permission as Permission);
        
        // Admin should have all privacy permissions
        expect(hasPermission('admin')).toBe(true, `Admin should have privacy permission: ${permission}`);
  ...
  describe('Permission Schema Validation', () => {
    ...
    it('should validate permission uniqueness across roles', () => {
      ...
      Object.entries(permissions).forEach(([role, permissions]) => {
        ...
        expect(uniqueRolePermissions.size).toBe(permissions.length, 
    ...
    it('should ensure minimum required permissions per role', () => {
      ...
      Object.entries(minimumPermissions).forEach(([role, minCount]) => {
        ...
        expect(rolePermissions.length).toBeGreaterThanOrEqual(minCount, 
  ...
  describe('Dynamic Permission Scenarios', () => {
      ...
      const complexOperations = [
        {
          ...
          role: 'admin' as Role,
          requiredPermissions: ['read:bill', 'create:bill', 'update:bill', 'delete:bill'] as Permission[]
        },
        {
          name: 'Building Management',
          role: 'manager' as Role,
          requiredPermissions: ['read:building', 'create:building', 'update:building'] as Permission[]
        },
        {
    ...
    it('should verify invitation management permissions across roles', () => {
      const invitationPermissions: Permission[] = [
        'read:invitation', 'create:invitation', 'update:invitation',
        'cancel:invitation', 'resend:invitation', 'audit:invitation'
      ];

      // Admin should have all invitation permissions
      invitationPermissions.forEach(permission => {
        expect(checkPermission(permissions, 'admin', permission))
          .toBe(true, `Admin should have invitation permission: ${permission}`);
      });

      // Manager should have most invitation permissions except audit
      const managerInvitationPerms = ['read:invitation', 'create:invitation', 'update:invitation', 'cancel:invitation', 'resend:invitation'];
      managerInvitationPerms.forEach(permission => {
        expect(checkPermission(permissions, 'manager', permission as Permission))
          .toBe(true, `Manager should have invitation permission: ${permission}`);
      });
  ...
  describe('Security Integration Testing', () => {
    it('should validate all permission categories are covered', () => {
      const expectedCategories = [
        'user', 'organization', 'building', 'residence', 'bill', 'budget',
        'maintenance_request', 'document', 'audit_log', 'notification',
        'invitation', 'feature', 'actionable_item', 'ai_analysis'
      ];
      ...
      expectedCategories.forEach(category => {
        expect(resourcesCovered.has(category))
    ...
    it('should ensure role consistency across all permissions', () => {
      ...
      allResources.forEach(resource => {
        ...
        if (managerResourcePerms.length > 0 && adminResourcePerms.length < managerResourcePerms.length) {
