...
describe('RBAC System Integration Tests', () => {
  ...
  describe('Permission Matrix Validation', () => {
    it('should validate comprehensive permission matrix against actual endpoints', async () => {
      ...
      const adminPermissions = validationResults.filter(
        (r) => r.role === 'admin' && r.hasPermission
      );
      const managerPermissions = validationResults.filter(
        (r) => r.role === 'manager' && r.hasPermission
      );
      const tenantPermissions = validationResults.filter(
        (r) => r.role === 'tenant' && r.hasPermission
      );
      ...
      console.warn(
        `✅ Validated permission matrix across ${validationResults.length} role-endpoint combinations`
      );
    });
    ...
    it('should verify no orphaned permissions exist', async () => {
      ...
      const unusedPermissions = uniquePermissions.filter(
        (permission) => permissionUsage[permission] === 1
      );
  ...
  describe('Security Edge Cases', () => {
    it('should prevent session hijacking attempts', async () => {
      ...
      const validResponse = await request(app).get('/api/auth/user').set('Cookie', adminCookies);

      expect(validResponse.status).toBe(200);

      // Test with manipulated cookie
      const manipulatedCookie = adminCookies[0].replace(
        /koveo\.sid=([^;]+)/,
        'koveo.sid=invalid-session-id'
      );
    ...
    it('should handle concurrent permission checks', async () => {
      ...
      const concurrentRequests = Array.from({ length: 10 }, (_, i) =>
        request(app).get('/api/auth/user').set('Cookie', adminCookies)
      );
    ...
    it('should validate role consistency across session lifecycle', async () => {
      ...
      const firstResponse = await request(app).get('/api/auth/user').set('Cookie', cookies);
      ...
      const secondResponse = await request(app).get('/api/auth/user').set('Cookie', cookies);
