...
describe('Task #842 — xlsx screening succeeds (no api_error)', () => {
  ...
  it('screens an xlsx buffer without degrading to api_error', async () => {
    // Build a real minimal xlsx buffer using ExcelJS so we exercise the
    // full extraction path in loadFileForClaude.
    const ExcelJS = await import('exceljs');
    const workbook = new ExcelJS.default.Workbook();
    const ws = workbook.addWorksheet('Carnet entretien');
    ws.addRows([
  ...
  it('marks an .xls (legacy binary format) upload as unsupported_mime', async () => {
    // ExcelJS does not support the old binary .xls format
    // (application/vnd.ms-excel). Uploads with that MIME type should
    // receive fallbackReason: 'unsupported_mime' so the UI shows the
    // correct warning instead of a false api_error badge.
    const buffer = Buffer.from('dummy-xls-content');
  ...
  it('strips control characters from xlsx sheet content before sending to AI prompt', async () => {
    ...
    const workbook = new ExcelJS.default.Workbook();
    const ws = workbook.addWorksheet('Controls');
    ws.addRows([
      ['Date', 'Notes'],
