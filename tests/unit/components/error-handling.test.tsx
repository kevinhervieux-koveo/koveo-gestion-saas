...
describe('Error Handling Component Tests', () => {
  ...
  describe('Property List Error Handling', () => {
    ...
    it('should display error message when fetch fails', async () => {
      ...
      expect(screen.getByTestId('property-error-message')).toHaveTextContent(
  ...
  describe('User Form Validation and Error Handling', () => {
    ...
    it('should show validation error for invalid Quebec phone format', async () => {
      ...
      expect(screen.getByTestId('error-phone')).toHaveTextContent(
        'Format de téléphone invalide (ex: +1-514-555-0123)'
      );
    });
    ...
    it('should handle server errors gracefully', async () => {
      ...
      expect(screen.getByTestId('error-submit')).toHaveTextContent(
        "Erreur serveur: Impossible de sauvegarder l'utilisateur"
      );
    });
  ...
  describe('Quebec-Specific Error Scenarios', () => {
    ...
    it('should handle Quebec postal code validation errors', () => {
      ...
      expect(screen.getByTestId('postal-error')).toHaveTextContent(
        'Code postal québécois invalide (ex: H1A 1A1)'
      );
    ...
    it('should handle Quebec property management specific errors', () => {
      ...
      expect(screen.getByTestId('quebec-error-0')).toHaveTextContent(
        'Violation du code du logement du Québec'
      );
      expect(screen.getByTestId('quebec-error-1')).toHaveTextContent(
        'Augmentation de loyer non conforme à la Régie du logement'
      );
      expect(screen.getByTestId('quebec-error-2')).toHaveTextContent(
        'Bail résidentiel invalide selon la loi québécoise'
      );
      expect(screen.getByTestId('quebec-error-3')).toHaveTextContent(
        'Délai de préavis insuffisant (24h requis au Québec)'
      );
    });
  });
