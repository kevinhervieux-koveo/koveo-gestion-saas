...
const validateQuebecCompliance = (
  text: string,
  language: 'en' | 'fr'
): {
  isCompliant: boolean;
  ...
  if (language === 'fr') {
    ...
    quebecFrenchRules.anglicismsToAvoid.forEach(({ wrong, correct }) => {
      if (wrong.test(text)) {
        ...
        suggestions.push(
          `Use "${correct}" instead of "${wrong.source.replace(/[\\^$*+?.()|[\]{}]/g, '\\$&')}"`
        );
      }
    });
...
const validateLaw25Terminology = (
  enText: string,
  frText: string
): {
  isValid: boolean;
...
describe('Quebec Law 25 Compliance and French Language Tests', () => {
  describe('Law 25 Terminology Validation', () => {
    it('should validate critical privacy terms are properly translated', () => {
      ...
      criticalTerms.forEach((termKey) => {
        ...
        expect(frTerm.toLowerCase()).not.toMatch(
          new RegExp(enTerm.toLowerCase().replace(/[aeiou]/g, '[àáâãäåæéèêëîíìïôòóõöøùúûüý]'), 'i')
        );
      });
    });
    ...
    it('should validate breach notification terminology', () => {
      ...
      const frText =
        'Nous devons signaler cette atteinte à la protection des renseignements personnels dans les 72 heures';
    ...
    it('should validate consent terminology consistency', () => {
      ...
      consentTerms.forEach(({ en, fr: _fr }) => {
        ...
        const hasEnProperty =
          globalThis.Object.prototype.hasOwnProperty.call(quebecComplianceTerms.en || {}, _key) ||
          globalThis.Object.prototype.hasOwnProperty.call(
            quebecComplianceTerms.en || {},
            en.replace(' ', '')
          );
        const hasFrProperty =
          globalThis.Object.prototype.hasOwnProperty.call(quebecComplianceTerms.fr || {}, _key) ||
          globalThis.Object.prototype.hasOwnProperty.call(
            quebecComplianceTerms.fr || {},
            en.replace(' ', '')
          );
  ...
  describe('Quebec French Language Quality', () => {
    it('should detect and suggest Quebec French alternatives to anglicisms', () => {
      ...
      const hasDownloadSuggestion = validation.suggestions.some(
        (suggestion) =>
          suggestion.includes('téléchargement') &&
          (suggestion.includes('download') || suggestion.includes('\\bdownload\\b'))
      );
    ...
    it('should validate proper French quotation marks usage', () => {
      ...
      expect(
        englishValidation.suggestions.filter((s) => s.includes('quotation')).length
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe('Legal Document Compliance', () => {
    it('should validate privacy policy terms in both languages', () => {
      const privacyPolicyTerms = {
        ...
        fr: {
          ...
          contact:
            'Contactez notre responsable de la protection des renseignements personnels pour toute question',
        },
      };
    ...
    it('should validate breach notification language', () => {
      ...
      expect(breachNotifications.fr).toContain(
