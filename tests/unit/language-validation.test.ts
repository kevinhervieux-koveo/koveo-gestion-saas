...
class LanguageValidator {
    ...
    function extractStrings(obj: unknown, path: string = ''): string[] {
      ...
      if (typeof obj === 'string') {
        ...
        Object.keys(obj).forEach((key) => {
          strings.push(
            ...extractStrings((obj as Record<string, unknown>)[key], path ? `${path}.${key}` : key)
          );
        });
      }
  ...
  generateReport(): string {
    ...
    if (errors.length === 0 && warnings.length === 0) {
      report +=
        '✅ Aucune violation détectée. Le contenu respecte les standards du français québécois.\n';
      return report;
    }
...
describe('Quebec French Language Validation', () => {
  ...
  it('should pass validation for proper Quebec French text', () => {
    const validText =
      "Veuillez vous connecter à votre tableau de bord de gestion d'immeuble au Québec";
