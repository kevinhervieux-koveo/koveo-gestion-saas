 ...
 *
 * End-to-end (component-level) test for task #1450 — verifies that when a
 * super_admin clicks the delete icon next to a Koveo system tag in the
 * admin Document Tags page, the DELETE call fires, a success toast is
 ...
 * a non-super-admin viewing the same system tags card sees the "Read-only"
 * cell instead of any action buttons — so a future refactor that drops
 * `onDelete={isSuperAdmin ? removeTag : undefined}` on the system tags
 * table would fail this test.
 *
 * Why this complements existing coverage:
 *   - `tests/unit/api/document-tags-delete-system-guard.test.ts` only
 ...
 *     `invalidateQueries` call inside `removeTag` triggers a refetch that
...
jest.mock('@/hooks/use-language', () => ({
  useLanguage: () => ({
    ...
    // page calls `t('dtDeleteConfirm', { name: tag.name })`.
    t: (key: string, values?: Record<string, unknown>) => {
      ...
      // asserting on. The real translation file uses `Delete tag "{name}"?`
      // but we don't load translations here; suffixing the values is enough
      // for the assertion `toContain(name)` to pass.
...
// Minimal Tabs mock (mirrors the access-control test).
...
describe('Document Tags admin page — super_admin can delete system tags end-to-end (task #1450)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('clicking delete on a Koveo system tag opens the in-app confirmation dialog, fires DELETE on confirm, removes the row, and shows a success toast', async () => {
    mockAuthState.user = USERS.super_admin;

    // apiRequest mock: when DELETE hits the system tag, drop it from the store
    // so the next refetch (triggered by invalidateQueries) returns no rows.
    mockApiRequest.mockImplementation(async (method: string, url: string) => {
      if (method === 'DELETE' && url === `/api/document-tags/${SYSTEM_TAG.id}`) {
        tagStore = tagStore.filter((t) => t.id !== SYSTEM_TAG.id);
        return { ok: true };
      }
    ...
    // The system tag row should be visible to the super_admin first.
    await findByTestId(`row-tag-${SYSTEM_TAG.id}`);
    const deleteBtn = getByTestId(`button-delete-tag-${SYSTEM_TAG.id}`);
    ...
    // The body text references the deleted tag's name.
    ...
    expect(mockApiRequest).toHaveBeenCalledWith(
      'DELETE',
      `/api/document-tags/${SYSTEM_TAG.id}`,
    );

    // Success toast was triggered.
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'dtToastDeletedTitle' }),
      );
    });

    // After the refetch, the row is no longer rendered.
    await waitFor(() => {
      expect(queryByTestId(`row-tag-${SYSTEM_TAG.id}`)).not.toBeInTheDocument();
    });
  ...
  it('aborts the DELETE if the super_admin cancels the in-app confirmation dialog (regression — confirm guard)', async () => {
    ...
    await act(async () => {
      fireEvent.click(getByTestId(`button-delete-tag-${SYSTEM_TAG.id}`));
    });
    ...
    expect(mockApiRequest).not.toHaveBeenCalled();
    // Row is still there.
    expect(getByTestId(`row-tag-${SYSTEM_TAG.id}`)).toBeInTheDocument();
    // Dialog has closed.
    await waitFor(() => {
      expect(queryByTestId('dialog-confirm-delete')).not.toBeInTheDocument();
    });
  ...
  it('renders a "Read-only" cell instead of edit/delete buttons for a non-super-admin (regression — guard wiring)', async () => {
    ...
    // Wait until the system tag row renders.
    const row = await findByTestId(`row-tag-${SYSTEM_TAG.id}`);
    ...
    // tree for a non-super-admin even if some refactor wired buttons by mistake.
    expect(mockApiRequest).not.toHaveBeenCalledWith(
      'DELETE',
      `/api/document-tags/${SYSTEM_TAG.id}`,
    );

    // Reference unused `getByTestId` so the destructured names mirror the
    // first test for readability — no-op assertion.
