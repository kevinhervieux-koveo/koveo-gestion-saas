  ...
  describe('Long Press Interactions', () => {
    it('should handle long press for context menus', async () => {
      const {
        ...
        ContextMenuContent,
        ContextMenuItem,
      } = require('../../client/src/components/ui/context-menu');

      render(
        <ContextMenu>
          <ContextMenuTrigger asChild>
            <div className='p-4 border rounded cursor-pointer'>Property: Maple Heights Condos</div>
          </ContextMenuTrigger>
          <ContextMenuContent>
  ...
  describe('Scroll Behavior', () => {
    it('should provide smooth scrolling for long property lists', () => {
      ...
      const scrollContainer = screen
        .getByText('Property 1: Building Unit #1A')
          ...
          <div className='p-4 space-y-4'>
            ...
            {Array.from({ length: 15 }, (_, i) => (
              <div key={i} className='p-3 border rounded'>
                Request #{i + 1}: {i % 3 === 0 ? 'Plumbing' : i % 3 === 1 ? 'Electrical' : 'HVAC'}{' '}
                issue
              </div>
            ))}
          </div>
