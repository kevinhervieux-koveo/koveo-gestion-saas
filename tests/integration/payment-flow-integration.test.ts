...
describe('Payment Flow Integration Tests', () => {
  ...
  describe('End-to-End Payment Processing', () => {
    it('should create consistent bill and money flow entries', async () => {
      ...
      expect(Math.abs(parseFloat(newFlow[0].amount.toString()))).toBe(1500.00);
    });
    ...
    it('should handle recurrent payment scenarios', async () => {
      ...
      const recurrentBill = await db
        ...
        .values({
          ...
          costs: [800.00, 800.00, 800.00], // Quarterly payments
          totalAmount: 2400.00,
          startDate: '2025-01-01',
          status: 'sent',
          createdBy: testUserId
      ...
      expect(totalFlowAmount).toBe(2400.00);
    });
    ...
    it('should validate payment status transitions', async () => {
      ...
      await db
        .update(bills)
        .set({ status: 'sent' })
        .where(eq(bills.id, testBill[0].id));

      const sentBill = await db
        .select()
        .from(bills)
        .where(eq(bills.id, testBill[0].id))
        .limit(1);
    ...
    it('should validate financial reconciliation', async () => {
      ...
      const incomeFlow = await db
        ...
        .values({
          ...
          amount: 2500.00, // Slightly more than expense
    ...
    it('should validate overdue payment handling', async () => {
      ...
      await db
        ...
        .values({
          ...
          amount: -1890.00, // Original + 5% penalty
