...
const mockRequest = (method: string, path: string, user: any, body?: any, query?: any) => ({
  ...
  file: method === 'POST' ? { path: '/tmp/test-file.pdf', originalname: 'test.pdf', mimetype: 'application/pdf' } : undefined
});
...
describe('Document API Integration Tests', () => {
  ...
  describe('GET /api/documents - Document Listing', () => {
    ...
    it('should filter documents by residence for residents', async () => {
      ...
      const req = mockRequest('GET', '/api/documents', resident, {}, { residenceId: 'residence-123' });
      const res = mockResponse();
  ...
  describe('GET /api/documents/:id/file - File Serving', () => {
    ...
    it('should handle both view and download modes', async () => {
      ...
      // View mode
      const viewReq = mockRequest('GET', '/api/documents/:id/file', resident, {}, { download: 'false' });
      const viewRes = mockResponse();
      expect(viewRes.setHeader).toHaveBeenCalledWith('Content-Disposition', expect.stringContaining('inline'));
      
      // Download mode
      const downloadReq = mockRequest('GET', '/api/documents/:id/file', resident, {}, { download: 'true' });
      const downloadRes = mockResponse();
      expect(downloadRes.setHeader).toHaveBeenCalledWith('Content-Disposition', expect.stringContaining('attachment'));
    });
  ...
  describe('Tenant Visibility Filtering', () => {
    it('should filter out documents not visible to tenants in residence view', async () => {
      const tenant = { id: 'tenant-1', role: 'tenant', email: 'tenant@test.com' };
      const req = mockRequest('GET', '/api/documents', tenant, {}, { residenceId: 'residence-123' });
      const res = mockResponse();
  ...
  describe('Cross-Residence Security', () => {
    it('should prevent access to documents from other residences', async () => {
      ...
      const req = mockRequest('GET', '/api/documents', resident1, {}, { residenceId: 'other-residence-456' });
      const res = mockResponse();
