  ...
  describe('Complete Demand Lifecycle', () => {
    it('should complete the full workflow: Create -> Review -> Approve -> Comment -> Complete', async () => {
      ...
      const demandData = {
        type: 'maintenance',
        description:
          'Kitchen sink faucet is leaking and needs immediate repair. Water is dripping constantly.',
      ...
      const publicCommentResponse = await request(app)
        ...
        .send({
          content:
            'We have scheduled a plumber for tomorrow between 2-4 PM. Please ensure someone is available to provide access.',
          isInternal: false,
        })
        .set(
          'test-user',
      ...
      const residentCommentResponse = await request(app)
        ...
        .send({
          content:
            'Thank you! I will be available tomorrow afternoon. The building entrance code is 1234.',
          isInternal: false,
        })
        .set(
          'test-user',
    ...
    it('should handle demand rejection workflow', async () => {
      ...
      const rejectResponse = await request(app)
        ...
        .send({
          status: 'rejected',
          reviewNotes:
            'Personal preferences for common area colors cannot be accommodated. Per building regulations, cosmetic changes require board approval and majority consent.',
        })
        .set(
          'test-user',
          JSON.stringify({
      ...
      const explanationComment = await request(app)
        ...
        .send({
          content:
            'If you would like to propose changes to common areas, please submit a formal proposal to the building committee for consideration at the next monthly meeting.',
          isInternal: false,
        })
        .set(
          'test-user',
    ...
    it('should handle demand cancellation by resident', async () => {
      ...
      const residentComment = await request(app)
        ...
        .send({
          content:
            'Never mind, I found the information on the building website. Pool hours are 6 AM - 10 PM daily.',
          isInternal: false,
        })
        .set(
          'test-user',
