...
describe('Bill Relationships Integration Tests', () => {
  ...
  describe('Parent-Child Relationship Creation', () => {
    it('should create proper parent-child relationships', async () => {
      ...
      const [parentInDb] = await db.select().from(bills).where(eq(bills.id, parentBill.id));
    ...
    it('should handle multiple parent bills with different children', async () => {
      ...
      const children1 = await db.select().from(bills).where(eq(bills.reference, parentBill1.id));

      const children2 = await db.select().from(bills).where(eq(bills.reference, parentBill2.id));
  ...
  describe('Cascade Delete Operations', () => {
    it('should delete only unpaid bills by default', async () => {
      ...
      const allChildren = await db.select().from(bills).where(eq(bills.reference, parentBill.id));
  ...
  describe('Relationship Data Integrity', () => {
    it('should maintain referential integrity with foreign key constraints', async () => {
      ...
      const children = await db.select().from(bills).where(eq(bills.reference, parentBill.id));
    ...
    it('should handle orphaned bills gracefully', async () => {
      ...
      const orphans = await db
        .select()
        .from(bills)
        .where(and(eq(bills.autoGenerated, true), eq(bills.reference, 'non-existent-parent-id')));

      expect(orphans.length).toBe(1);
      expect(orphans[0].reference).toBe('non-existent-parent-id');

      // Verify we can query orphaned bills
      const allAutoBills = await db.select().from(bills).where(eq(bills.autoGenerated, true));

      const orphanedBills = allAutoBills.filter(
        (bill) => bill.reference === 'non-existent-parent-id'
      );
    ...
    it('should prevent circular references', async () => {
      ...
      try {
        ...
        const [selfRef] = await db.select().from(bills).where(eq(bills.id, 'bill-self-ref'));
  ...
  describe('Complex Relationship Scenarios', () => {
    it('should handle bills with mixed auto-generated and manual flags', async () => {
      ...
      const autoGeneratedBills = await db
        .select()
        .from(bills)
        .where(and(eq(bills.buildingId, mockBuilding.id), eq(bills.autoGenerated, true)));

      const manualBills = await db
        .select()
        .from(bills)
        .where(and(eq(bills.buildingId, mockBuilding.id), eq(bills.autoGenerated, false)));
    ...
    it('should handle regeneration of children after parent changes', async () => {
      ...
      const firstChildren = await db.select().from(bills).where(eq(bills.reference, parentBill.id));
