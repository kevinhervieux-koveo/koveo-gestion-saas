...
describe('Common Spaces Demo Organization Scenarios', () => {
  ...
  describe('Time Limit Validation System', () => {
    ...
    beforeAll(async () => {
      ...
      testUserId =
        REAL_DEMO_USERS.admin?.id || REAL_DEMO_USERS.resident?.id || REAL_DEMO_USERS.tenant?.id;
    ...
    afterEach(async () => {
      ...
      try {
        await db.execute(
          sql`DELETE FROM bookings WHERE user_id = ${testUserId} AND start_time > NOW()`
        );
      } catch (error) {
        // Ignore cleanup errors
      }
  ...
  describe('Booking Download Scenarios', () => {
    it('should validate ICS calendar export format', () => {
      const userBookings = [
        ...
        {
          id: 'booking-3',
          spaceName: 'Salle de Fête',
          startTime: '2024-03-03T19:00:00Z',
          endTime: '2024-03-03T22:00:00Z',
        },
      ];
  ...
  describe('Manager User Restriction Scenarios', () => {
    ...
    it('should validate blocked user booking prevention', () => {
      ...
      const restriction = userRestrictions.find(
        (r) =>
  ...
  describe('Demo Organization Data Validation', () => {
    ...
    it('should validate French space names', () => {
      const frenchNames = REAL_DEMO_COMMON_SPACES.filter(
        (s) => s.name.includes('Salle') || s.name.includes('Entrepôt')
      );
...
function getCategoryFromName(name: string): string {
  ...
  if (name.toLowerCase().includes('réunion') || name.toLowerCase().includes('meeting')) {
    return 'meeting';
  }
  if (name.toLowerCase().includes('fête') || name.toLowerCase().includes('party')) {
    return 'event';
  }
  if (name.toLowerCase().includes('lavage') || name.toLowerCase().includes('laundry')) {
    return 'utility';
  }
  if (name.toLowerCase().includes('entrepôt') || name.toLowerCase().includes('storage')) {
    return 'storage';
  }
  return 'other';
}
