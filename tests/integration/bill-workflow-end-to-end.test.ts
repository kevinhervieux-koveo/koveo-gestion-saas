...
describe('Bill Workflow End-to-End Integration Tests', () => {
  ...
  describe('Complete Recurrent Bill Lifecycle', () => {
    it('should handle full lifecycle: create -> generate -> update -> pay -> delete', async () => {
      ...
      const updatedBills = await db.select().from(bills).where(eq(bills.reference, parentBill.id));
    ...
    it('should handle complex split payment workflow', async () => {
      ...
      const months2to3 =
        (thirdDate.getFullYear() - secondDate.getFullYear()) * 12 +
  ...
  describe('Multi-Year Projection Workflow', () => {
    it('should handle 25-year bill projection and management', async () => {
      ...
      const sortedBills = generatedBills.sort(
    ...
    it('should handle regeneration after parent changes', async () => {
      ...
      // Initial generation
      await request(app).post(`/api/bills/${modifiableBill.id}/generate-future`).expect(200);
      ...
      await request(app).post(`/api/bills/${generatedBills[0].id}/mark-paid`).expect(200);

      await request(app).post(`/api/bills/${generatedBills[1].id}/mark-paid`).expect(200);
    ...
    it('should handle large dataset operations efficiently', async () => {
      ...
      await Promise.all(
        generatedBills.map((bill) =>
          request(app).post(`/api/bills/${bill.id}/mark-paid`).send({ paymentDate: '2024-02-15' })
        )
      );
  ...
  describe('Real-World Scenario Simulation', () => {
    it('should simulate property management company workflow', async () => {
      ...
      const results = await Promise.all(
        bills.map((bill) => request(app).post(`/api/bills/${bill.id}/generate-future`).expect(200))
      );
      ...
      const allStats = await Promise.all(
        bills.map((bill) => request(app).get(`/api/bills/${bill.id}/generated-stats`).expect(200))
      );
      ...
      const januaryBills = allGeneratedBills.filter((bill) => bill.startDate.startsWith('2024-01'));

      await Promise.all(
        januaryBills.map((bill) =>
          request(app).post(`/api/bills/${bill.id}/mark-paid`).send({
      ...
      const totalPaidBills = finalStats.reduce(
        (sum, stats) => sum + stats.body.generatedBills.paidBills,
        0
      );
  ...
  describe('Performance and Monitoring', () => {
    it('should maintain performance standards under load', async () => {
      ...
      await request(app).post(`/api/bills/${performanceBill.id}/generate-future`).expect(200);
      ...
      await request(app).get(`/api/bills/${performanceBill.id}/generated-stats`).expect(200);
