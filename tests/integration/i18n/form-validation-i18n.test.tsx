  ...
  const validateField = (name: string, _value: string): string => {
    switch (name) {
      ...
      case 'description':
        ...
        if (value.length < 10) {
          return validation.minLength(10);
        }
        if (value.length > 2000) {
          return validation.maxLength(2000);
        }
        return '';
      case 'contactEmail':
        if (!_value) {
...
describe('Form Validation Internationalization Tests', () => {
    ...
    it('should validate phone format for Quebec in English', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('phone-error')).toHaveTextContent(
          'Please enter a valid Quebec phone number (format: 514-555-0123)'
        );
      });
    });
  ...
  describe('French Form Validation', () => {
    ...
    it('should validate phone format for Quebec in French', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('phone-error')).toHaveTextContent(
          'Veuillez entrer un numéro de téléphone québécois valide (format: 514-555-0123)'
        );
      });
    });
  ...
  describe('Loading and Submit States', () => {
    it('should display loading state in English', async () => {
      ...
      await user.type(
        ...
        'This is a maintenance request for testing'
      );
    ...
    it('should display loading state in French', async () => {
      ...
      await user.type(
        screen.getByTestId('description-textarea'),
        'Ceci est une demande de maintenance pour test'
      );
      await user.type(screen.getByTestId('email-input'), 'test@example.com');
      await user.type(screen.getByTestId('phone-input'), '514-555-0123');
