      ...
      for (let month = 1; month <= 12; month++) {
        ...
        for (const category of BILL_CATEGORIES) {
          ...
          for (let billIndex = 0; billIndex < billsToCreate; billIndex++) {
            ...
            try {
              const paymentService = new PaymentGenerationService();
              await paymentService.generatePaymentsForBill(bill.id);
              console.log(`   ✅ Generated payments for bill ${billNumber}`);
            } catch (paymentError) {
              console.warn(`   ⚠️ Failed to generate payments for bill ${billNumber}:`, paymentError);
              // Continue with bill creation even if payment generation fails
            }
