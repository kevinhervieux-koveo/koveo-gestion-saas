...
`;
  ...
  if (includeIntegration) {
    if (process.env.GEMINI_API_KEY) {
      ...
      integrationTestResult = runTests(
        'tests/integration/ai-form-response-validation.test.ts',
        verbose
      );
...
function main() {
  ...
  if (args.integration) {
    if (process.env.GEMINI_API_KEY) {
      console.warn(
        `${colors.green}🔑 GEMINI_API_KEY detected - including integration tests${colors.reset}`
      );
    } else {
      console.warn(
        `${colors.yellow}⚠️  GEMINI_API_KEY not set - integration tests will be skipped${colors.reset}`
      );
    }
  }
