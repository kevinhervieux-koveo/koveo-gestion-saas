...
async function runQualityMetricTests(): Promise<QualityTestResult> {
  ...
  try {
    ...
    const unitTestResult = execSync('npm test tests/unit/quality-metrics.test.ts -- --verbose', {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
    ...
    console.warn(
      `   Success Rate: ${result.testsTotal > 0 ? Math.round((result.testsPassed / result.testsTotal) * 100) : 0}%`
    );
  } catch (_error) {
...
async function validateSystemEffectiveness(): Promise<void> {
  ...
  if (systemHealth.metricScores.length > 0) {
    ...
    systemHealth.metricScores.forEach((score) => {
      ...
      console.warn(
        `   ${status} ${score.metric}: ${score.accuracy.toFixed(1)}% accuracy (${score.measurementCount} measurements)`
      );
    });
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.warn(
      `   Status: ${testResult.success ? '✅ All systems operational' : '❌ Issues detected'}`
    );
