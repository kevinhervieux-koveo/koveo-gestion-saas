...
async function fixFile(filePath: string): Promise<void> {
  ...
  content = content.replace(/\b(match|context|error|event|index|key|value|props|params|options|data|result|_response)\b(?=\s*[:)])/g, '_$1');
  ...
  if (missingDescriptionPattern.test(content)) {
    ...
    content = content.replace(missingDescriptionPattern, `/**\n * ${functionName.charAt(0).toUpperCase() + functionName.slice(1)} function.\n$1`);
  }
  
  // Add missing @returns for functions with return types
  const missingReturnsPattern = /\/\*\*[\s\S]*?\*\/\s*(?:export\s+)?(?:async\s+)?function\s+\w+[^{]*:\s*([^{]+)\s*\{/;
  if (missingReturnsPattern.test(content) && !content.includes('@returns')) {
    ...
    if (returnTypeMatch) {
      ...
      if (returnType !== 'void') {
        content = content.replace(/(\*\/\s*(?:export\s+)?(?:async\s+)?function)/, ` * @returns ${returnType.includes('Promise') ? 'Promise resolving to result' : 'Function result'}.\n$1`);
      }
    }
  }
