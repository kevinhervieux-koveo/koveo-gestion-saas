...
class QuebecSecurityChecker {
  ...
  private async checkPersonalDataProtection(): Promise<void> {
    ...
    if (dataClassificationIssues.length > 0) {
      this.addResult(
        ...
        'FAIL',
        'Potential sensitive data exposure found',
        dataClassificationIssues
      );
      this.criticalIssues++;
    } else {
      this.addResult(
  ...
  private async checkDataEncryption(): Promise<void> {
    ...
    if (existsSync(packageJsonPath)) {
      ...
      const missingLibs = encryptionLibs.filter(
        (lib) => !deps[lib] && !Object.keys(deps).some((dep) => dep.includes(lib))
      );
    ...
    if (encryptionIssues.length > 0) {
      this.addResult(
        'Data Encryption',
        'FAIL',
        'Encryption compliance issues found',
        encryptionIssues
      );
      this.criticalIssues++;
    } else {
      this.addResult('Data Encryption', 'PASS', 'Data encryption properly implemented');
    }
  }
  ...
  private async checkAccessControls(): Promise<void> {
    ...
    if (accessControlIssues.length > 0) {
      this.addResult('Access Controls', 'FAIL', 'Access control issues found', accessControlIssues);
      this.criticalIssues++;
    } else {
      this.addResult('Access Controls', 'PASS', 'Access controls properly implemented');
    }
  }
  ...
  private async checkDataStorageCompliance(): Promise<void> {
    ...
    for (const file of clientFiles) {
      ...
      const sensitiveStoragePattern =
        /(localStorage|sessionStorage).*\.(set|getItem).*?(personal|private|sensitive|password|token|user)/i;
    ...
    if (storageIssues.length > 0) {
      this.addResult(
        ...
        'FAIL',
        'Data storage compliance issues found',
        storageIssues
      );
      this.criticalIssues++;
    } else {
  ...
  private async checkPrivacyByDesign(): Promise<void> {
    ...
    if (privacyIssues.length > 0) {
      this.addResult('Privacy by Design', 'FAIL', 'Privacy by design issues found', privacyIssues);
      this.criticalIssues++;
    } else if (this.results[this.results.length - 1]?.status !== 'WARNING') {
      this.addResult('Privacy by Design', 'PASS', 'Privacy by design principles implemented');
    }
  }
