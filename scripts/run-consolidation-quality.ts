...
async function checkCodeConsolidation(): Promise<QualityMetric> {
  ...
  try {
    ...
    if (categoriesFiles.length > 3) {
      issues.push(
        `Found ${categoriesFiles.length} files with DOCUMENT_CATEGORIES - should use consolidated version`
      );
    } else {
    ...
    if (loadingFiles.length > 10) {
      issues.push(
        `Found ${loadingFiles.length} manual loading states - consider using useLoadingState hook`
      );
    } else {
...
async function checkHookPatterns(): Promise<QualityMetric> {
  ...
  try {
    ...
    if (deleteMutations.length > 5 && mutationFiles.length < deleteMutations.length / 2) {
      issues.push(
        'Many delete handlers but few consolidated mutations - consider using useDeleteMutation'
      );
    }
